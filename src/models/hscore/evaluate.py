"""
H-Score walk-forward evaluation.

Compares three scoring variants on the test set:
    1. current_formula — original H-Score components (approximated)
    ...
    for alias, addr in KNOWN_WALLETS.items():
        ...
        if wallet_df.empty:
            ...
            summary_rows.append({
                ...
                "avg_h_score": float("nan"), "avg_rank_pct": float("nan"),
        ...
        print(f"{'─' * 65}")
        print(f"  {'date':<13} {'h_score':>8}  {'rank':>5}/{'pool':>5}  {'rank_pct':>9}  {'label':>5}")
        for _, r in wallet_df.iterrows():
            print(
                f"  {r['snapshot_date']:<13} {r['h_score']:>8.3f}  "
