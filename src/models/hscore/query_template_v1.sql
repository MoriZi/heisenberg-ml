...
snap_7d AS (
  SELECT
    ...
    m7.best_trade           AS best_trade_7d,
...
eligible AS (
  SELECT
    ...
    COALESCE(s1.worst_trade_1d,            0) AS worst_trade_1d,
    ...
    COALESCE(s7.best_trade_7d,             0) AS best_trade_7d,
...
ranked AS (
  SELECT
    *,
    ...
    PERCENT_RANK() OVER (ORDER BY best_trade ASC)                 AS pr_best_trade,
    PERCENT_RANK() OVER (ORDER BY best_trade_7d ASC)              AS pr_best_trade_7d,
    PERCENT_RANK() OVER (ORDER BY avg_position_size ASC)          AS pr_avg_position_size,
    ...
    PERCENT_RANK() OVER (ORDER BY perfect_entry_count ASC)        AS pr_perfect_entry_count,
    ...
    PERCENT_RANK() OVER (ORDER BY worst_trade DESC)               AS pr_worst_trade_inv,
    PERCENT_RANK() OVER (ORDER BY worst_trade_1d DESC)            AS pr_worst_trade_1d_inv,
...
scored AS (
  SELECT
    *,
    ROUND((
      pr_total_pnl                 *  3.152855 +
      pr_total_pnl_1d              *  1.756282 +
      pr_total_pnl_3d              *  0.516558 +
      pr_total_pnl_7d              *  4.212733 +
      pr_total_invested            *  0.625174 +
      pr_total_invested_3d         *  0.641673 +
      pr_total_invested_7d         * 13.604210 +
      pr_best_trade                *  0.492499 +
      pr_best_trade_7d             *  0.665377 +
      pr_avg_position_size         *  0.513880 +
      pr_stddev_position_size_7d   *  3.301844 +
      pr_dominant_market_pnl       *  1.471644 +
      pr_dominant_market_pnl_7d    *  1.685918 +
      pr_pnl_cat_sports            *  2.359121 +
      pr_pnl_cat_other             *  0.356743 +
      pr_pnl_cat_other_7d          *  0.591257 +
      pr_perfect_entry_count       *  6.533361 +
      pr_statistical_confidence    *  5.617252 +
      pr_statistical_confidence_1d *  2.167692 +
      pr_markets_traded            *  6.433453 +
      pr_pnl_cat_economics         *  0.200906 +
      pr_worst_trade_inv           *  2.225732 +
      pr_worst_trade_1d_inv        *  3.206464 +
      pr_roi_1d_inv                *  4.882484 +
      pr_roi_3d_inv                *  2.148502 +
      pr_profit_factor_inv         *  1.846541 +
      pr_profit_factor_7d_inv      *  0.367885 +
      pr_win_rate_inv              *  2.611314 +
      pr_win_rate_1d_inv           *  0.000000 +
      pr_win_rate_3d_inv           *  4.990345 +
      pr_market_conc_inv           *  1.681952 +
      pr_pnl_cat_crypto_inv        *  0.000000 +
      pr_sortino_ratio             *  0.805036 +
      pr_calmar_ratio              *  1.816348 +
      pr_gain_to_pain_ratio        *  9.276961 +
      pr_annualized_return         *  3.130230 +
      pr_total_trades              *  4.109775
