"""
Sport H-Score weight optimizer using scipy SLSQP.

Objective: maximize mean Precision@K across snapshot dates.
...
def compute_p_at_k(
    ...
    return -make_p_at_k_objective(X, y, date_groups, k)(weights)
...
def optimize(
    ...
    Returns dict with keys: weights, p_at_k, result.
    """
    rng = np.random.default_rng(config.seed)
    obj = make_p_at_k_objective(X, y, date_groups, config.k)
    ...
    for i in range(config.n_init):
        ...
        if (i + 1) % 10 == 0:
            print(f"  init {i + 1:>3}/{config.n_init}  best P@{config.k} = {best_p_at_k:.4f}")

    return {"weights": best_result.x, "p_at_k": best_p_at_k, "result": best_result}
...
def build_weight_table(
    weights: np.ndarray, config: SportHScoreConfig
) -> pd.DataFrame:
