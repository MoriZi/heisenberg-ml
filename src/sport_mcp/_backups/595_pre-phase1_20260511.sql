...
base AS (
    SELECT
        ...
        COALESCE(g.sortino_ratio, 1.0094) AS sortino_ratio,
        COALESCE(g.annualized_return, 3.18675) AS annualized_return,
...
scored AS (
    SELECT proxy_wallet, total_pnl, sports_pnl, sports_trades, win_rate,
        ...
        ROUND((
            ...
            PERCENT_RANK() OVER (ORDER BY total_invested_7d ASC)        *  9.582 +
            PERCENT_RANK() OVER (ORDER BY win_rate_7d DESC)             *  8.275 +
            PERCENT_RANK() OVER (ORDER BY sports_trades ASC)            *  7.153 +
            PERCENT_RANK() OVER (ORDER BY best_market_pnl_7d ASC)       *  7.141 +
            PERCENT_RANK() OVER (ORDER BY worst_market_pnl_7d DESC)     *  6.996 +
            ...
            PERCENT_RANK() OVER (ORDER BY pnl_cat_sports_7d ASC)        *  3.291 +
            PERCENT_RANK() OVER (ORDER BY sports_invested ASC)          *  3.024 +
            PERCENT_RANK() OVER (ORDER BY worst_market_pnl DESC)        *  2.038 +
            PERCENT_RANK() OVER (ORDER BY sports_pnl_mlb ASC)           *  2.005 +
            PERCENT_RANK() OVER (ORDER BY sortino_ratio ASC)            *  1.820 +
            PERCENT_RANK() OVER (ORDER BY sports_pnl ASC)               *  1.809 +
            PERCENT_RANK() OVER (ORDER BY sports_pnl_la_liga ASC)       *  1.760 +
            PERCENT_RANK() OVER (ORDER BY sports_pnl_ligue_1 ASC)      *  1.108 +
